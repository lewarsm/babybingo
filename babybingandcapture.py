...
def get_random_filename(extension='.png'):
    ...
    random_filename = ''.join(random.choice(letters) for i in range(10)) + extension
    return random_filename

def save_screenshot():
    # Hide the main window
    root.withdraw()
    # Capture only the Baby Bingo frame
    x = root.winfo_rootx()
    y = root.winfo_rooty()
    ...
    # Show a message box with the filename
...
def create_tkinter_app():
    global cells, bingo_card, logo_img, announcement_label, root
    ...
    screenshot_button = tk.Button(root, text="Save Screenshot", command=save_screenshot)
    screenshot_button.grid(row=8, column=2, columnspan=2, pady=10)
